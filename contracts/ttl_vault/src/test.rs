...
fn test_beneficiary_veto_release_condition_blocks_trigger_release_before_expiry() {
    ...
    client.veto_release_condition(&vault_id, &beneficiary).unwrap();
...
fn test_beneficiary_veto_can_be_revoked_before_expiry() {
    ...
    client.veto_release_condition(&vault_id, &beneficiary).unwrap();
    client.revoke_veto_release_condition(&vault_id, &beneficiary).unwrap();
...
fn test_beneficiary_veto_does_not_apply_after_vault_expired() {
    ...
    let result = client.try_veto_release_condition(&vault_id, &beneficiary);
    assert!(result.is_err());
...
fn test_non_primary_beneficiary_cannot_veto() {
    ...
    let result = client.try_veto_release_condition(&vault_id, &other);
    assert!(result.is_err());
...
fn test_beneficiary_vault_limit_enforced() {
    ...
    assert_eq!(err, soroban_sdk::Error::from_contract_error(ContractError::BeneficiaryCapacityExceeded as u32));
}
